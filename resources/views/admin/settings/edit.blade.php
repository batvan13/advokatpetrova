    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" novalidate>
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm divide-y divide-gray-100">
            ...
            <div class="px-8 py-6 space-y-5">
                ...
                <p class="text-xs font-bold tracking-widest uppercase text-gray-400">Консултации — Контакти</p>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">

                    {{-- consultation_phone_number --}}
                    <div>
                        <label for="consultation_phone_number" class="block text-sm font-medium text-gray-700 mb-1.5">
                            Телефон за консултации
                        </label>
                        <input
                            type="tel"
                            id="consultation_phone_number"
                            name="consultation_phone_number"
                            value="{{ old('consultation_phone_number', $settings->get('consultation_phone_number')) }}"
                            placeholder="+359 88 888 8888"
                            class="w-full px-3 py-2 border rounded text-sm text-gray-900 placeholder-gray-400
                                   focus:outline-none focus:border-gray-500 transition-colors
                                   {{ $errors->has('consultation_phone_number') ? 'border-red-300 bg-red-50' : 'border-gray-200' }}"
                        >
                        <p class="mt-1.5 text-xs text-gray-400">Номерът, на който клиентите се обаждат за телефонна консултация.</p>
                        @error('consultation_phone_number')
                            <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="consultation_viber_number" class="block text-sm font-medium text-gray-700 mb-1.5">
                            Viber номер за консултации
                        </label>
                        <input
                            type="tel"
                            id="consultation_viber_number"
                            name="consultation_viber_number"
                            value="{{ old('consultation_viber_number', $settings->get('consultation_viber_number')) }}"
                            placeholder="+359 88 888 8888"
                            class="w-full px-3 py-2 border rounded text-sm text-gray-900 placeholder-gray-400
                                   focus:outline-none focus:border-gray-500 transition-colors
                                   {{ $errors->has('consultation_viber_number') ? 'border-red-300 bg-red-50' : 'border-gray-200' }}"
                        >
                        <p class="mt-1.5 text-xs text-gray-400">Номерът за Viber видео консултации (може да е различен от телефонния).</p>
                        @error('consultation_viber_number')
                            <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
