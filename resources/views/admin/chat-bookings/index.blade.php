...
@section('title', 'Чат консултации — Admin')

@section('content')

    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Онлайн чат консултации</h1>
            <p class="mt-1 text-sm text-gray-500">Записани чат консултации.</p>
        </div>
        <a href="{{ route('admin.chat-bookings.archived') }}"
    ...
    @if ($bookings->isEmpty())
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm px-8 py-16 text-center">
            ...
            <p class="text-sm text-gray-400">Все още няма направени чат консултации.</p>
        </div>
    @else
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100">
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Сесия</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($bookings as $booking)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-4">
                                @if ($booking->status === \App\Models\ChatConsultationBooking::STATUS_BOOKED)
                                    <span class="inline-flex items-center rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-700">Записан</span>
                                @elseif ($booking->status === \App\Models\ChatConsultationBooking::STATUS_COMPLETED)
