...
@section('title', 'Редакция на период — Admin')

@section('content')

    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Редакция на период</h1>
            <p class="mt-1 text-sm text-gray-600">
                {{ $closure->starts_at->format('d.m.Y') }} — {{ $closure->ends_at->format('d.m.Y') }}
            </p>
        </div>
    ...
    <form action="{{ route('admin.closures.update', $closure) }}" method="POST" novalidate>
        @csrf
        @method('PUT')
