...
@section('title', 'Viber видео консултация')
@section('description', 'Изберете продължителност, ден и час за Viber видео консултация с адвокат Петрова.')
...
<div class="consultation-page">
    ...
    <section class="relative overflow-hidden">
        ...
        <div class="relative z-10 mx-auto max-w-4xl px-4 pt-16 pb-16">
            ...
            <div class="mt-10 rounded-xl border border-petrova-gold/20 bg-petrova-deep/70 backdrop-blur-sm px-6 py-6 space-y-4">
                <div>
                    ...
                    <p class="text-sm text-petrova-secondary/80 leading-relaxed">
                        Изберете продължителност на консултацията — 30 или 60 минути — след което изберете удобен ден и час.
                        Консултацията се провежда чрез Viber видео разговор. В избрания ден и час ще получите обаждане от адвокатското дружество.
                    </p>
                </div>
                <div>
                    <p class="text-base font-semibold italic text-petrova-gold mb-1">Важно</p>
                    <p class="text-sm text-petrova-secondary/80 leading-relaxed">
                        Моля, бъдете на разположение в записания час. При закъснение от над 5–10 минути, консултацията може да бъде съкратена или отменена.
            ...
            <form
                id="viber-form"
                action="{{ route('viber-consultation.submit') }}"
                method="POST"
                novalidate
            >
                @csrf
                ...
                <input type="hidden" name="duration"       id="f-duration"       value="{{ old('duration', '30') }}">
                ...
                {{-- ══════════════════════════════════════════════════
                     STEP 1 — Duration toggle + Calendar + Slot picker
                ══════════════════════════════════════════════════ --}}
                <div id="step-1" class="mt-6">
                    ...
                    <div class="rounded-xl border border-petrova-gold/20 bg-petrova-deep/70 backdrop-blur-sm px-4 py-6 sm:px-6">
                        ...
                        {{-- Duration toggle --}}
                        <div class="mb-6">
                            <p class="text-sm font-semibold italic text-petrova-gold mb-3">Продължителност</p>
                            <div class="inline-flex rounded border border-petrova-gold/30 overflow-hidden" role="group" aria-label="Продължителност на консултацията">
                                <button
                                    type="button"
                                    id="dur-30"
                                    data-duration="30"
                                    class="px-5 py-2 text-sm font-semibold transition-colors bg-petrova-gold text-petrova-deep"
                                    aria-pressed="true"
                                >
                                    30 мин
                                </button>
                                <button
                                    ...
                                    id="dur-60"
                                    data-duration="60"
                                    class="px-5 py-2 text-sm font-semibold transition-colors text-petrova-secondary/70 hover:text-petrova-primary hover:bg-petrova-gold/10"
                                    aria-pressed="false"
                                >
                                    60 мин
                                </button>
                            ...
                            {{-- Price display --}}
                            <p id="duration-price" class="mt-2 text-sm text-petrova-secondary/70">
                                @if ($pricing)
                                    <span id="price-30">
                                        {{ number_format((float) $pricing->price_eur, 2, ',', '.') }}&nbsp;€@if($pricing->show_bgn_price && $pricing->price_bgn) / {{ number_format((float) $pricing->price_bgn, 2, ',', '.') }}&nbsp;лв.@endif
                                    </span>
                                    <span id="price-60" class="hidden">
                                        @if($pricing->price_eur_60)
                                            {{ number_format((float) $pricing->price_eur_60, 2, ',', '.') }}&nbsp;€@if($pricing->show_bgn_price && $pricing->price_bgn_60) / {{ number_format((float) $pricing->price_bgn_60, 2, ',', '.') }}&nbsp;лв.@endif
                                        @else
                                            —
                                        @endif
                                    </span>
                ...
                <div id="step-3" class="mt-6 hidden">
                    ...
                    <div class="rounded-xl border border-petrova-gold/20 bg-petrova-deep/70 backdrop-blur-sm px-6 py-6 space-y-4 mb-6">
                        <div>
                            <p class="text-base font-semibold italic text-petrova-gold mb-1">Инструкции</p>
                            <p class="text-sm text-petrova-secondary/80 leading-relaxed">
                                Прегледайте данните преди да потвърдите заявката. В избрания ден и час ще получите Viber видео обаждане от адвокатското дружество.
                            </p>
                        </div>
                        <div>
                            <p class="text-base font-semibold italic text-petrova-gold mb-1">Важно</p>
                            <p class="text-sm text-petrova-secondary/80 leading-relaxed">
                                Консултацията се провежда изцяло чрез Viber видео разговор. Всички разговори са конфиденциални.
                            </p>
                        </div>
                    </div>
                        ...
                        <div class="rounded-xl border border-petrova-gold/20 bg-petrova-deep/60 px-5 py-5">
                            ...
                            <div class="space-y-3 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-petrova-secondary/70">Viber видео консултация</span>
                                    <span id="review-duration" class="text-petrova-primary font-medium"></span>
                                </div>
                    ...
                    <div class="mt-4 rounded-xl border border-petrova-gold/20 bg-petrova-deep/60 px-5 py-4 space-y-3 text-sm">
                        ...
                        <div class="flex justify-between border-t border-petrova-gold/10 pt-3">
                            ...
                            <span id="review-price" class="text-petrova-primary font-semibold"></span>
                        </div>
                    </div>
...
(function () {
    ...
    // ── Constants ────────────────────────────────────────────────────
    var SLOTS_URL   = '{{ route('viber-consultation.slots') }}';
    ...
    // Prices passed from Blade (used in review panel).
    var PRICES = {
        30: {
            eur: '{{ $pricing ? number_format((float) $pricing->price_eur, 2, ",", ".") : "" }}',
            bgn: '{{ ($pricing && $pricing->show_bgn_price && $pricing->price_bgn) ? number_format((float) $pricing->price_bgn, 2, ",", ".") : "" }}',
        },
        60: {
            eur: '{{ ($pricing && $pricing->price_eur_60) ? number_format((float) $pricing->price_eur_60, 2, ",", ".") : "" }}',
            bgn: '{{ ($pricing && $pricing->show_bgn_price && $pricing->price_bgn_60) ? number_format((float) $pricing->price_bgn_60, 2, ",", ".") : "" }}',
        },
    };
    ...
    var currentSlots = [];
    var selectedDuration = parseInt(document.getElementById('f-duration').value, 10) || 30;
    ...
    var dur30Btn = document.getElementById('dur-30');
    var dur60Btn = document.getElementById('dur-60');
    var price30  = document.getElementById('price-30');
    var price60  = document.getElementById('price-60');
    ...
    function setDuration(dur) {
        ...
        var active   = 'bg-petrova-gold text-petrova-deep';
        var inactive = 'text-petrova-secondary/70 hover:text-petrova-primary hover:bg-petrova-gold/10';

        if (dur === 30) {
            dur30Btn.className = dur30Btn.className.replace(inactive, '').trim() + ' ' + active;
            dur60Btn.className = dur60Btn.className.replace(active, '').trim() + ' ' + inactive;
            dur30Btn.setAttribute('aria-pressed', 'true');
            dur60Btn.setAttribute('aria-pressed', 'false');
            if (price30) { price30.classList.remove('hidden'); }
            if (price60) { price60.classList.add('hidden'); }
        } else {
            dur60Btn.className = dur60Btn.className.replace(inactive, '').trim() + ' ' + active;
            dur30Btn.className = dur30Btn.className.replace(active, '').trim() + ' ' + inactive;
            dur60Btn.setAttribute('aria-pressed', 'true');
            dur30Btn.setAttribute('aria-pressed', 'false');
            if (price60) { price60.classList.remove('hidden'); }
            if (price30) { price30.classList.add('hidden'); }
        }

        // If a date is already selected, reload slots for new duration.
        if (selectedDate) {
            selectedSlot = null;
            fSlot.value  = '';
    ...
    dur30Btn.addEventListener('click', function () { setDuration(30); });
    dur60Btn.addEventListener('click', function () { setDuration(60); });
    ...
    function loadSlots(iso) {
        ...
        var url = SLOTS_URL + '?date=' + encodeURIComponent(iso) + '&duration=' + selectedDuration;
    ...
    btnToStep3.addEventListener('click', function () {
        ...
        if (!firstName || !lastName || !email || !phone || !payment || !consent.checked) {
            document.getElementById('viber-form').submit();
            return;
        }

        // Populate review panel
        document.getElementById('review-duration').textContent    = selectedDuration + ' минути';
        ...
        document.getElementById('review-price').textContent       = buildPriceString(selectedDuration);
    ...
    // Restore duration toggle state (e.g. after server validation failure)
