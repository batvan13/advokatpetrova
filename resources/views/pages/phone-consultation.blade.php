...
@section('title', 'Телефонна консултация')
@section('description', 'Изберете удобен ден и час за телефонна консултация с адвокат Петрова.')
    ...
    <section class="relative overflow-hidden">
        ...
        <div class="relative z-10 mx-auto max-w-4xl px-4 pt-16 pb-16">
            ...
            <nav class="mb-8 flex items-center gap-2 text-sm text-petrova-secondary/70" aria-label="Breadcrumb">
                ...
                <span aria-hidden="true">/</span>
                <span class="text-petrova-secondary/50">Телефонна консултация</span>
            </nav>
            ...
            <div class="mt-10 rounded-xl border border-petrova-gold/20 bg-petrova-deep/70 backdrop-blur-sm px-6 py-6 space-y-4">
                <div>
                    ...
                    <p class="text-sm text-petrova-secondary/80 leading-relaxed">
                        След успешно заплащане ще получите имейл с потвърждение на часа. В избрания ден и час е необходимо Вие да се свържете с
                        адвокатското дружество на посочения телефонен номер. Консултацията започва при установяване на връзка.
                    </p>
                </div>
                <div>
                    <p class="text-base font-semibold italic text-petrova-gold mb-1">Важно</p>
                    <p class="text-sm text-petrova-secondary/80 leading-relaxed">
                        Моля, обадете се точно в резервирания час. При закъснение от над 5–10 минути, консултацията може да бъде съкратена или отменена.
                        Консултацията се провежда изцяло по телефон.
                    </p>
                </div>
            </div>
            ...
            <form
                id="phone-form"
                action="{{ route('phone-consultation.submit') }}"
                method="POST"
                novalidate
            >
                @csrf
                ...
                <div id="step-2" class="mt-6 hidden">
                    ...
                    <div class="rounded-xl border border-petrova-gold/20 bg-petrova-deep/70 backdrop-blur-sm px-6 py-6 space-y-5">
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                @foreach (\App\Models\PhoneConsultationBooking::PAYMENT_METHODS as $value => $label)
                ...
                <div id="step-3" class="mt-6 hidden">
                    ...
                    <div class="rounded-xl border border-petrova-gold/20 bg-petrova-deep/70 backdrop-blur-sm px-6 py-6 space-y-4 mb-6">
                        <div>
                            <p class="text-base font-semibold italic text-petrova-gold mb-1">Инструкции</p>
                            <p class="text-sm text-petrova-secondary/80 leading-relaxed">
                                След успешно заплащане ще получите имейл с потвърждение на часа. Ще се свържем с Вас на посочения телефон в избрания час.
                                Моля, бъдете на разположение в записания час.
                            </p>
                        </div>
                        <div>
                            <p class="text-base font-semibold italic text-petrova-gold mb-1">Важно</p>
                            <p class="text-sm text-petrova-secondary/80 leading-relaxed">
                                Консултацията се провежда изцяло по телефон. Всички разговори са конфиденциални.
                            </p>
                        </div>
                    </div>
                        ...
                        <div class="rounded-xl border border-petrova-gold/20 bg-petrova-deep/60 px-5 py-5">
                            ...
                            <div class="space-y-3 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-petrova-secondary/70">Телефонна консултация</span>
...
(function () {
    ...
    // ── Constants ────────────────────────────────────────────────────
    var SLOTS_URL   = '{{ route('phone-consultation.slots') }}';
    ...
    var currentSlots = [];     // last loaded slot list
    ...
    btnToStep3.addEventListener('click', function () {
        ...
        if (!firstName || !lastName || !email || !phone || !payment || !consent.checked) {
            // Let server-side handle the actual error messages; just scroll to top
    ...
    // If returning from a failed server submission, restore state
    var oldDate = fDate.value;
    var oldSlot = fSlot.value;

    if (oldDate) {
        selectedDate = oldDate;
        // Parse view month/year from stored date
    ...
    if (oldDate) {
        ...
        // On server-side error, jump straight to step 2 if we had a slot
        @if ($errors->any())
